...
\begin{frame}{Objectifs de la modélisation}
  \begin{block}{Objectifs}
	...
	\begin{itemize}
		\footnotesize
		\item Représentation des états intermédiaires non nécessaires
		\item<7-> Pas de reproduction des phénomènes antérieurs
\end{itemize}	
\end{block}}
\uncover<8->{\begin{block}{Requis}
\begin{itemize}
		\footnotesize
		\item Utilisation de modèles préexistants
		\item<9> Solutions libres et open-source pour une recherche \textbf{vérifiable} et \textbf{reproductible}
	\end{itemize}
\end{block}}
\end{frame}
...
\begin{frame}{État de l'art des modèles de développement résidentiel}
	\only<1>{Les modèles d'\textbf{occupation du sol} (Batty 97, Arentze 08, Mustapha 18). %todo ajouter images?! peut être en mettre un tout petit peu plus
		\begin{itemize}
			\item Simulation changement d'état et l'urbanisation 
	...
	\only<2>{
		Les modèles d'\textbf{étude du marché immobilier} (UrbanSimul ()) permettent de~:%TODO ref
		\begin{itemize}
		...
		\begin{itemize}
			...
			\item Peu adaptés pour représenter la complexité des mécanismes multi-échelles d'un développement résidentiel 
...
\uncover<2->{	
	\begin{block}{}
		\textbf{Utilisations}\\
		Permettre de comparer plusieurs \textbf{scénarios} de développement résidentiels\\	
		Représenter des futurs hypothètiques, recherchés, redoutés\\
...
\begin{frame}{ArtiScales : Rappel des objectifs}
		...
		\begin{itemize}
			...
			\item soumis aux contraintes d'aménagements isolées %(reprsentant diffrents mécanismes et objectifs)
...
\begin{frame}{ArtiScales : Fonctionnement}
	...
	%todo ajouter une étape de validation sur chaque flèche
\end{frame}
...
\begin{frame}{Variabilité de MUP-City}
		...
		\end{block}}
	\end{block}
	%todo ajouter la prise en main du modèle pour automatiser les 
\end{frame}
...
%todo definitly décomposer cette algo et travailler sur le lien avec les mécanismes
...
\begin{frame}{Formes prédéterminées des bâtiments}
	...
	\end{block}}
\only<6>{Type de contraintes non native - temps de calculs importants}
%todo illu d'un ordi qui mouline
\end{frame}
...
\begin{frame}{Construction du modèle ArtiScales}
	...
	%todo ajouter les processus de validation nécessaires
\end{frame}
...
\begin{frame}{Validation de différentes configurations spatiales de \\MUP-City}
	...
	\begin{itemize}
		\item Nombre de cellules selon et leurs localisations
...
{\footnotesize exemple de l'application d'un décalage de 20 mètres à la grille de décomposition}}}
	\only<4>{
		\begin{block}{Scénario de \textbf{développement} résidentiel \textbf{ciblé}}
			\begin{itemize}
...
\only<5>{\centering{\includegraphics[width=10cm]{Images/MUP/ManuN6St-prez.png}\\
	...
	\only<6>{
		\begin{block}{Scénario d'\textbf{intensification} de la densité résidentielle}
			\begin{itemize}
...
\subsection{Évaluation du fonctionnement d'ArtiScales}
\begin{frame}
\subsectionpage
\end{frame}
